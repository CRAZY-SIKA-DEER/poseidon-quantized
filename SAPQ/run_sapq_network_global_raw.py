...
def load_sapq_sensitivity(cfg: PPQConfig, device: torch.device):
    ...
    if "layer_sensitivity_raw" not in obj:
        raise ValueError(f"'layer_sensitivity_raw' not found in: {sens_path}")

    sens_dict = {
        name: tensor.to(device)
        for name, tensor in obj["layer_sensitivity_raw"].items()
    }
...
def maybe_load_or_compute_ranges(cfg, model, frozen_iter, candidate_layers, device):
    ...
    if model_name == "NS-PwC-L":
        ranges_path = Path(cfg.repo_root) / "precalculated_ranges" / model_name / "p1e-04" / "ranges.pt"
    else:
        ranges_path = Path(cfg.repo_root) / "precalculated_ranges" / model_name / "best" / "ranges.pt"
...
def main():
    ...
    # steps_path = out_dir / "raw"/"sapq_global_step_sizes.pt"
    # history_path = out_dir / "raw"/"sapq_global_history.json"
    # results_path = out_dir / "raw"/"sapq_global_results.json"
    epoch_dir = out_dir / "raw" / f"epoch_{cfg.num_epochs}"
    epoch_dir.mkdir(parents=True, exist_ok=True)

    steps_path = epoch_dir / "sapq_global_step_sizes.pt"
    history_path = epoch_dir / "sapq_global_history.json"
    results_path = epoch_dir / "sapq_global_results.json"
