# SAPQ/debug_like_prior_grad_by_layer.py
...
SAVE_DIR = REPO / "SAPQ/debug_grad_outputs/NS-SVS-L"
...
def collect_layer_grad_stats(step_sizes_dict, ranges_dict, sens_dict, grad_type, batch_idx):
    rows = []
    ...
    for name, entry in step_sizes_dict.items():
        ...
        bit_push = g / torch.clamp(step, min=EPS)

        # g > 0 => Adam/SGD decreases S => bitwidth up
        # g < 0 => Adam/SGD increases S => bitwidth down
        up_mask = g > 0
        down_mask = g < 0
        ...
        rows.append({
            ...
            "frac_push_bits_up": up_mask.float().mean().item(),
            "frac_push_bits_down": down_mask.float().mean().item(),

            "sum_g_over_s": bit_push.sum().item(),
            "mean_g_over_s": bit_push.mean().item(),
            "abs_mean_g_over_s": bit_push.abs().mean().item(),
