...
def compute_mc_loss_single_batch(model,
    ...
    for name in target_layers:
        ...
        # a_step is currently a placeholder tensor; we do NOT use it for noise now.
        ...
        for _ in range(num_mc_samples):
            # --- No activation noise: X stays clean ---
            X_noisy = X_clean

            # --- Still add uniform noise to weights per out_channel ---
...
def compute_mc_loss_with_prior(
    ...
    """
    Network-wise Monte Carlo likelihood loss (PPQ) + MDL prior **for a single batch**.

    Likelihood:
        Match final network output y_noisy to cached clean y_clean for that batch:
            E_mc [ ||y_noisy - y_clean||^2 / (2*eta) ]

    Prior:
        MDL prior on WEIGHT step sizes only (same as your compute_mdl_prior).

    Args:
        model: Poseidon model
        step_sizes_dict: {layer_name: (w_step, a_step)} learnable step sizes (a_step ignored in likelihood)
        frozen_batches: list of frozen calibration batches (dicts)
        clean_net_outputs: list of cached clean final outputs (CPU tensors), aligned with frozen_batches
        ranges_dict: output of compute_data_ranges_poseidon
        batch_idx: which frozen batch to use
        num_mc_samples: MC samples
    ...
    Returns:
        ...
        likelihood_loss: network-wise MC likelihood term (tensor with grad)
        prior_loss:      MDL prior term (tensor with grad)
    """
    # Likelihood: network-wise MC loss using cached clean final outputs
    ...
    # MDL prior on weight step sizes (unchanged)
...
def get_lr_for_epoch(epoch: int, base_lr: float = 1e-6, num_epochs: int = 4) -> float:
    """
...
def optimize_step_sizes(
    ...
    # ---- 4) Cache clean network outputs (final output) ----
    ...
    ## ---- 5) Find compatible layers (range shapes must match Linear in/out) ----
    target_layers = []
    for name in cand_layers:
        ...
        a_ranges = rec.get("activation_ranges", None)  # should match in_features
        w_ranges = rec.get("weight_ranges", None)      # should match out_features
        if a_ranges is None or w_ranges is None:
            continue

        mod = name2mod[name]  # nn.Linear
    ...
    print(f"\nStarting optimization: epochs={num_epochs}, mc_samples={num_mc_samples}, "
          f"eta={eta}, gamma={gamma}, base_lr={lr}, updates_per_batch={updates_per_batch}, average_cap= {avg_cap_lam}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx in range(num_batches):
            for _ in range(updates_per_batch):
                ...
                if avg_cap_bits is not None:
                    avg_cap_prior = prior_weighted_avg_bits_cap_poseidon(
                        step_sizes_dict=step_sizes_dict,
                        ranges_dict=ranges_dict,
                        channel_weights=channel_weights,   # you already computed this
                        target_bits=float(avg_cap_bits),
                        lam=float(avg_cap_lam),
                        alpha=float(avg_cap_alpha),
                    )

                total_loss = like_loss + prior_loss + avg_cap_prior
        ...
        if epoch % log_every == 0 or epoch == 1 or epoch == num_epochs:
            ...
            print(f"... Prior={prior_loss.item():.6f} | CapPrior={avg_cap_prior.item():.6f} | AvgBits={avg_bits:.2f}")
...
def prior_weighted_avg_bits_cap_poseidon(
    ...
    """
    Penalize ONLY if parameter-weighted avg bits > target_bits.
    Smooth hinge using softplus. Differentiable.
    """
    # compute_avg_bits currently returns a Python float (uses .item()) -> NOT differentiable.
    # So we need a differentiable version (tensor) here.

    # Differentiable avg bits:
    total_bits_weighted = None
    total_weight = None

    for name, (w_step, a_step) in step_sizes_dict.items():
        rec = ranges_dict.get(name, None)
        if rec is None or w_step is None:
            continue
        w_range = rec["weight_ranges"].to(w_step.device)  # [out_features]
        bits = torch.log2((w_range + eps) / (w_step + eps))  # tensor [out_features]

        w = channel_weights[name].to(bits.device) if (channel_weights is not None and name in channel_weights) else torch.ones_like(bits)
        if w.numel() == 1:
            w = w.expand_as(bits)
    ...
    if total_weight is None or float(total_weight.detach().cpu()) == 0.0:
        # return 0 penalty (safe)
        device = next(p.device for pair in step_sizes_dict.values() for p in pair if p is not None)
        return torch.zeros((), device=device)

    bits_avg = total_bits_weighted / (total_weight + eps)  # tensor scalar
...
def get_clean_network_outputs_poseidon(model, frozen_batches, device):
    """
    Cache the *final network outputs* (clean / FP) for each frozen calibration batch.

    This is the network-wise analogue of get_clean_outputs_poseidon(), but instead of
    saving per-layer (X_pre, Y_post), it saves only the final model output tensor.

    Args:
        model: Poseidon/ScOT model (already loaded)
        frozen_batches: list of batches (dicts) already materialized in memory
                        e.g., frozen_batches = list(calib_iter()) inside optimize_step_sizes
        device: torch.device

    Returns:
        clean_net_outputs: list of torch.Tensor, length = len(frozen_batches)
                           Each tensor is the model final output for that batch,
                           stored on CPU (detached).
    """
    model.eval()

    clean_net_outputs = []

    # Use inference_mode to reduce memory; outputs are detached anyway.
    with torch.inference_mode():
        for batch_idx, batch in enumerate(frozen_batches):
            ...
            outputs = model(
                ...
                time=t,
                pixel_mask=pm,
                labels=y,   # can be None; model should handle it
            )

            # Poseidon returns an object; we want the final prediction tensor
            pred = outputs.output

            clean_net_outputs.append(pred.detach().cpu())

    print(f"✓ Cached clean network outputs for {len(clean_net_outputs)} frozen batches.")
    if len(clean_net_outputs) > 0:
        s0 = tuple(clean_net_outputs[0].shape)
        print(f"  First cached output shape: {s0}")
...
def compute_mc_loss_single_batch_network(
    ...
    """
    Network-wise Monte Carlo likelihood loss (PPQ) for a SINGLE frozen calibration batch.

    Goal:
        Match the *final network output* of the noisy model to the cached clean output:
            L = E_mc [ || y_noisy - y_clean ||^2 / (2*eta) ]

    Noise model:
        - weight-only noise (per output channel) using each layer's learnable step size
        - NO activation noise (consistent with your current setup)

    Important:
        - This function MUST run with gradients enabled (no torch.no_grad / inference_mode),
          because we need gradients w.r.t. step sizes.
        - We implement noise injection via forward hooks on nn.Linear modules.
          Each hook replaces that layer's output with a linear op using a noised weight.

    Args:
        model: Poseidon/ScOT model
        step_sizes_dict: {layer_name: (w_step, a_step)}  (a_step ignored here)
        frozen_batches: list of batch dicts (already materialized)
        clean_net_outputs: list of cached clean final outputs (CPU tensors), aligned with frozen_batches
        batch_idx: which batch to use
        num_mc_samples: Monte Carlo samples
        eta: PPQ noise scaling
        device: torch device or device string

    Returns:
        likelihood_loss: scalar tensor (requires grad)
    """
    if model is None:
        raise ValueError("model must not be None for network-wise loss.")
    ...
    if batch_idx < 0 or batch_idx >= len(frozen_batches):
        raise IndexError(f"batch_idx={batch_idx} out of range for frozen_batches (len={len(frozen_batches)})")
    if batch_idx < 0 or batch_idx >= len(clean_net_outputs):
        raise IndexError(f"batch_idx={batch_idx} out of range for clean_net_outputs (len={len(clean_net_outputs)})")

    batch = frozen_batches[batch_idx]
    y_clean = clean_net_outputs[batch_idx].to(device)  # [B, ...] final output

    # Prepare batch tensors (same as your poseidon forward)
    x  = batch["pixel_values"].to(device)
    t  = batch.get("time", None)
    pm = batch.get("pixel_mask", None)
    y  = batch.get("labels", None)  # labels might exist but not needed for likelihood
    ...
    # Use only valid Linear layers present in model
    target_layers = []
    ...
    if len(target_layers) == 0:
        raise ValueError("No valid nn.Linear layers found in step_sizes_dict for network-wise loss.")

    mc_losses = []

    # ---------------------------------------------------------------------
    # MC loop: each sample registers hooks -> forward -> compute loss -> remove hooks
    # ---------------------------------------------------------------------
    for _ in range(num_mc_samples):
        handles = []

        # Define hook factory
        def make_noisy_linear_hook(layer_name: str, w_step_tensor: torch.Tensor):
            def hook(mod: torch.nn.Linear, inp, out):
                # Input activation (we do NOT quantize activations here)
                x_in = inp[0]

                # Add uniform noise to weights per output channel
                # mod.weight: [out_features, in_features]
                W_clean = mod.weight
                W_noisy = add_quantization_noise(W_clean, w_step_tensor, channel_axis=0)

                # Use PyTorch linear with (x_in, W_noisy)
                y_out = torch.nn.functional.linear(x_in, W_noisy, mod.bias)
                return y_out
            return hook
        ...
        for lname in target_layers:
            ...
            if isinstance(w_step, torch.nn.Parameter):
                ...
                w_step_t = torch.as_tensor(w_step, device=device)
            w_step_t = w_step_t.to(device)

            # Shape safety: w_step must match out_features
            if w_step_t.numel() != mod.weight.size(0):
                # If mismatch, skip this layer (better than crashing mid-run)
                continue

            h = mod.register_forward_hook(make_noisy_linear_hook(lname, w_step_t))
            handles.append(h)

        # Forward pass through the entire model (with noisy weights via hooks)
        outputs = model(
            pixel_values=x,
            time=t,
            pixel_mask=pm,
            labels=y,   # keep likelihood purely reconstruction-based
        )
        y_noisy = outputs.output

        # MSE reconstruction at final output
    ...
    likelihood_loss = torch.stack(mc_losses).mean()
...
def main():
    ...
    dyn4_steps  = compute_dynamic_stepsizes(model, cand_layers, num_bits=4,  device=device)
    ...
    for gamma in gamma_list:
        ...
        step_sizes_dict, ranges_dict, history = optimize_step_sizes(
            ...
            updates_per_batch=1,
            return_ranges=True,
            eval_every=1,           # <--- evaluate PPQ every 4 epochs
