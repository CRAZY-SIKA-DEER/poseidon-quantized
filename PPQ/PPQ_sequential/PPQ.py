...
def compute_data_ranges(model, dataloader, device, percentile_prob=1e-4):
    ...
    # Pre-compute the erf_inv constant ON THE CORRECT DEVICE
    z = torch.sqrt(torch.tensor(2.0, device=device)) * torch.special.erfinv(
        torch.tensor(1.0 - 2.0 * percentile_prob, device=device)
    )
    erf_inv_value = float(z)  # Convert to Python float for efficiency
    ...
    with torch.no_grad():
        ...
        for name, module in model.named_modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                ...
                w_std = w_flat.std(dim=1, unbiased = False)    # [out_channels], uisng unbiased is consistnet with batch norm
        ...
        # ==============================
        for inputs, _ in dataloader:  # Loop over batches
            inputs = inputs.to(device)
            
            def get_hook(name):
                def hook(module, input, output):
                    x = input[0]  # Pre-op activation.   # Shape: [batch_size/num_of_samples_in_one_batch, channels, H, W]
                    
                    # Flatten per channel: [channels, -1]. [channels, batch_size * H * W]
                    x_flat = x.transpose(0, 1).reshape(x.size(1), -1)
                    
                    # Compute μ and σ per channel
                    x_mean = x_flat.mean(dim=1)  # [channels]
                    x_std = x_flat.std(dim=1, unbiased=False)   # [channels], uisng unbiased is consistnet with batch norm
        ...
        for name in ranges_dict:
            ...
            avg_mean = all_means.mean(dim=0).to(device) # Average across batches
            avg_std = all_stds.mean(dim=0).to(device) # Average across batches
...
# ========================
# helper function for finding the best percentile
# ========================

def evaluate_quantized_model(model, dataloader, ranges, device, num_bits=8):
    """
        ...
        for inputs, targets in dataloader:
            ...
            def get_quantize_hook(name, w_ranges, a_ranges):
                def hook(module, input, output):
                    ...
                    x_quantized = torch.round(x_flat / a_step) * a_step
                    x_quantized = x_quantized.reshape_as(x.transpose(0, 1)).transpose(0, 1)
                    
                    # Quantize weights
                    w_flat = module.weight.view(module.weight.size(0), -1)
                    w_step = w_ranges.view(-1, 1) / num_levels
                    w_quantized = torch.round(w_flat / w_step) * w_step
                    w_quantized = w_quantized.view_as(module.weight)
                    
                    # Compute output with quantized inputs and weights
                    if isinstance(module, nn.Conv2d):
                        y = F.conv2d(x_quantized, w_quantized, module.bias,
                                    module.stride, module.padding)
                    elif isinstance(module, nn.Linear):
                        y = F.linear(x_quantized, w_quantized, module.bias)
                    
                    return y
                return hook
            
            for name, module in model.named_modules():
                if isinstance(module, (nn.Conv2d, nn.Linear)) and name in ranges:
...
# ========================
def optimize_step_sizes(model, dataloader, ranges_dict=None, num_epochs=50, num_mc_samples=10, 
    ...
    # Compute or use provided data ranges
    if ranges_dict is None:
        print(f"Computing data ranges with percentile_prob={percentile_prob}...")
        # So we only need to pass the percentile_prob to calculate the range if ranges_dict is None
        ranges_dict = compute_data_ranges(model, dataloader, device, percentile_prob=percentile_prob)
    else:
        # If the range_dict is not None(it is passed in in mian, then we dont need tp pass the percentile_prob)
        print("Using provided data ranges...")
...
if __name__ == "__main__":
    ...
    # Compute ranges ONCE with best_P
    print(f"\nComputing ranges with best percentile {best_P}...")
    ranges_dict = compute_data_ranges(model, dataloader, device, percentile_prob=best_P)
    ...
    for gamma in [0.001, 0.004, 0.007]:
        ...
        step_sizes = optimize_step_sizes(
            model, dataloader, 
            ranges_dict=ranges_dict,  # ← Pass pre-computed ranges
            num_epochs=50, 
            num_mc_samples=10, 
            lr=1e-2,
            gamma=gamma,
            device=device,
            #percentile_prob=best_P  # No need to pass the found best_P, since it is already used in calculaitng the ranges_dict
        )
