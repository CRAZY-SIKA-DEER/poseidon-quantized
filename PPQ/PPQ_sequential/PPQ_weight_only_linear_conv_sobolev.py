...
# ------------------------------------------------------------------
# Sobolev H¹ loss for the LAST layer (zero + first-order terms)
# ------------------------------------------------------------------


def sobolev_loss(delta: torch.Tensor) -> torch.Tensor:
    """
    Approximate H^1 Sobolev norm of the error delta = u' - u.

    Supports:
      - delta shape (B, C, H, W)
      - delta shape (B, S)    -> interpreted as scalar field on sqrt(S) x sqrt(S)
    Returns a scalar (mean over batch, channels, space).
    """
    ...
    if delta.dim() == 4:
        ...
        # (B, S) -> assume scalar field on a square grid
        B, S = delta.shape
        H = int(math.sqrt(S))
        if H * H != S:
            raise ValueError(
                f"sobolev_loss: cannot reshape last dimension {S} into HxW square."
            )
        d = delta.view(B, 1, H, H)  # treat as single-channel scalar field
    else:
        raise ValueError(f"sobolev_loss: expected 2D or 4D tensor, got shape {delta.shape}")

    # 0th-order (L2) term
    l2_term = torch.mean(d ** 2)

    # First-order finite differences
    dx = d[:, :, :, 1:] - d[:, :, :, :-1]   # along x (width)
    dy = d[:, :, 1:, :] - d[:, :, :-1, :]   # along y (height)

    grad_term = torch.mean(dx ** 2) + torch.mean(dy ** 2)

    return l2_term + grad_term
...
def compute_mc_loss_single_batch(model,
                                 ...
                                 eta: float = 1e-4,
                                 device: str = 'cpu',
                                 last_layer_name: str = "patch_recovery.mixup"):
    """
    ...
    Modified:
        - For ALL layers except `last_layer_name`: Gaussian MSE likelihood
              loss_elem = mean((Y_noisy - Y_clean)^2) / (2 * eta)
        - For `last_layer_name` only: Sobolev H^1 likelihood
              loss_elem = sobolev_loss(Y_noisy - Y_clean) / (2 * eta)
    """
    ...
    # Ensure the chosen last_layer_name is actually among the target layers
    if last_layer_name not in target_layers:
        raise ValueError(
            f"last_layer_name='{last_layer_name}' is not in target_layers.\n"
            f"Available layers: {target_layers}"
        )
    ...
    for name in target_layers:
        ...
        mc_losses = []
        is_last_layer = (name == last_layer_name)
        ...
        if isinstance(module, torch.nn.Linear):
            ...
            for _ in range(num_mc_samples):
                ...
                delta = Y_noisy - Y_clean
                if is_last_layer:
                    # Sobolev likelihood on the last layer
                    loss_elem = sobolev_loss(delta) / (2 * eta)
                else:
                    # Standard Gaussian MSE likelihood
                    loss_elem = torch.mean(delta ** 2) / (2 * eta)
...
def compute_mc_loss_with_prior(
    ...
    likelihood_loss = compute_mc_loss_single_batch(
        ...
        eta=eta,
        device=device,
        last_layer_name = "patch_recovery.mixup"
    )
...
def main():
    ...
    results_path = os.path.join(artifacts_dir, "PwC-T-results-1-5-conv-sobolev.json")
    results = {}
    ...
    step_pt_path   = os.path.join(artifacts_dir, "ppq_step_sizes-1-5-conv-sobolev.pt")
    step_json_path = os.path.join(artifacts_dir, "ppq_step_sizes-1-5-conv-sobolev.json")
