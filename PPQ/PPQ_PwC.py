"""
Calibrate the Poseidon NS-PwC model with Probabilistic Programming Quantisation (PPQ).

This script
  * loads the ScOT Poseidon-L checkpoint for NS-PwC,
  * builds small calibration / validation splits from the assembled NS-PwC dataset,
  * runs the PPQ MAP optimisation from `PPQ.PPQ_L2`,
  * reports validation losses, learned step sizes, and (optionally) saves the quantised weights.

Example
-------
python -m PPQ.PPQ_PwC \
    --model-dir models/NS-PwC \
    --dataset-dir dataset/NS-PwC \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PPQ calibration on the NS-PwC Poseidon model.")
    parser.add_argument("--model-dir", type=Path, default=Path("models/NS-PwC"), help="Directory with config.json & pytorch_model.bin.")
    parser.add_argument("--dataset-dir", type=Path, default=Path("dataset/NS-PwC"), help="Directory containing NS-PwC.nc.")
    ...
    parser.add_argument("--calib-samples", type=int, default=128, help="Max calibration samples (after temporal expansion).")
    ...
    parser.add_argument("--batch-size", type=int, default=1, help="Batch size for calibration/validation loaders.")
    parser.add_argument("--num-workers", type=int, default=0, help="Dataloader workers.")

    parser.add_argument("--bit-width", type=int, default=4, help="PPQ quantisation bit-width.")
    parser.add_argument("--gamma", type=float, default=0.001, help="MDL prior strength.")
