#!/bin/bash -l
#SBATCH --job-name=launch_sbpq_lks_long
#SBATCH --nodes=1
#SBATCH --gpus=0
#SBATCH --time=1:00:00
#SBATCH --output=logs/launch_sbpq_lks_long-%j.out
#SBATCH --error=logs/launch_sbpq_lks_long-%j.err

cd /home/u6ey/yiheng.u6ey/poseidon-quantized
mkdir -p logs

RUN_GROUP="lr_kappa_steps_long"
            ...
            for kappa in "${BETA_KAPPAS[@]}"; do
                for steps in "${OPTIMIZATION_STEPS[@]}"; do
                    for lr in "${LEARNING_RATES[@]}"; do
                        ...
                        job_name="sbpq5_${model}_B${bits}_sob${sob_order}_k${kappa}_s${steps}_lr${lr_tag}"
...
echo "Submitted ${job_count} SBPQ long LR/kappa/steps sweep jobs for group=${RUN_GROUP}."
