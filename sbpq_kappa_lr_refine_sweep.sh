#!/bin/bash -l
#SBATCH --job-name=launch_sbpq_kappa_lr
#SBATCH --nodes=1
#SBATCH --gpus=0
#SBATCH --time=1:00:00
#SBATCH --output=logs/launch_sbpq_kappa_lr-%j.out
#SBATCH --error=logs/launch_sbpq_kappa_lr-%j.err

cd /home/u6ey/yiheng.u6ey/poseidon-quantized
mkdir -p logs

RUN_GROUP="kappa_lr_refine"
        ...
        for sob_order in "${SOBOLEV_ORDERS[@]}"; do
            for kappa in "${BETA_KAPPAS[@]}"; do
                for lr in "${LEARNING_RATES[@]}"; do
                    ...
                    job_name="sbpq2_${model}_B${bits}_sob${sob_order}_k${kappa}_lr${lr_tag}"
...
echo "Submitted ${job_count} SBPQ kappa/LR refinement sweep jobs for group=${RUN_GROUP}."
