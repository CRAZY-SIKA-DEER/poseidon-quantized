#!/bin/bash -l
#SBATCH --job-name=launch_sbpq_kls
#SBATCH --nodes=1
#SBATCH --gpus=0
#SBATCH --time=1:00:00
#SBATCH --output=logs/launch_sbpq_kls-%j.out
#SBATCH --error=logs/launch_sbpq_kls-%j.err

cd /home/u6ey/yiheng.u6ey/poseidon-quantized
mkdir -p logs

RUN_GROUP="kappa_lr_steps_sob_refine"
            ...
            for kappa in "${BETA_KAPPAS[@]}"; do
                for steps in "${OPTIMIZATION_STEPS[@]}"; do
                    for lr in "${LEARNING_RATES[@]}"; do
                        ...
                        job_name="sbpq3_${model}_B${bits}_sob${sob_order}_k${kappa}_s${steps}_lr${lr_tag}"
...
echo "Submitted ${job_count} SBPQ kappa/LR/steps/Sobolev refinement sweep jobs for group=${RUN_GROUP}."
